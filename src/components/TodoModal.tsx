...
export default function TodoModal({ todo, defaultStatus = 'pending', onClose, onSubmit }: Props) {
  ...
  useEffect(() => {
    if (todo) {
      ...
      reset({ title: '', description: '', status: defaultStatus, priority: 'medium', deadline: '' });
    }
  }, [todo, defaultStatus, reset]);
  ...
  return (
    ...
    >
      <div
        className="bg-[#F4F5F7] rounded-xl shadow-2xl w-full max-w-lg overflow-hidden"
      ...
      >
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="px-5 pb-5 flex flex-col gap-4">
          {/* Title */}
          <div>
